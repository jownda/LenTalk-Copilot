...
export async function splitImageSource(
  ...
  return await invoke('split_image_source', {
    ...
    cols,
    lineThickness,
    colFractions: colFractions && colFractions.length > 0 ? colFractions : undefined,
    rowFractions: rowFractions && rowFractions.length > 0 ? rowFractions : undefined,
  });
}
...
export async function mergeStoryboardImages(
  ...
  if (!isTauri()) {
    // 浏览器开发环境无 Tauri 后端，使用前端 Canvas 兜底实现，
    // 行为与 Rust merge_storyboard_images 保持一致（rows×cols 网格 + 文字叠加）。
    const { localMergeStoryboardImages } = await import(
      '@/features/canvas/application/localMerge'
    );
    return await localMergeStoryboardImages(payload);
  }
