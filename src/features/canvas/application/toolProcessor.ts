...
export class CanvasToolProcessor implements ToolProcessor {
  ...
  private async rotateImage(sourceImage: string, options: Record<string, unknown>): Promise<string> {
    // 编辑器已累积变换出结果时,直接使用该结果(所见即所得)
    const resultDataUrl = options.resultDataUrl;
    if (typeof resultDataUrl === 'string' && resultDataUrl.startsWith('data:')) {
    ...
    const operation = String(options.operation ?? 'rotate-right-90');
