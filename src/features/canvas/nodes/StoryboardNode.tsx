...
export const StoryboardNode = memo(({ id, data, selected, width, height }: StoryboardNodeProps) => {
  ...
  const handleReplaceFromLocalFile = useCallback(
    async (frameId: string) => {
      ...
      try {
        ...
        setExportError(error instanceof Error ? error.message : '替换图片失败');
      }
    },
    [id, prepareNodeImage, updateStoryboardFrame]
  );
    ...
    >
      ...
      {pickerState && typeof document !== 'undefined'
        ? createPortal(
          <div
            ref={pickerMenuRef}
            className="nowheel fixed z-[140] w-[176px] overflow-hidden rounded-xl border border-[rgba(255,255,255,0.16)] bg-surface-dark shadow-xl"
          ...
          >
            ...
            <button
              type="button"
              className="flex w-full items-center gap-2 border-t border-[rgba(255,255,255,0.12)] bg-bg-dark/70 px-2 py-2 text-left text-sm text-text-dark transition-colors hover:bg-bg-dark"
              onClick={(event) => {
                event.stopPropagation();
                void handleReplaceFromLocalFile(pickerState.frameId);
              }}
              title="从本地文件选择图片替换此格"
            >
