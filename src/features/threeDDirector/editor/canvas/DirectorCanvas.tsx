...
import { setMotionViewportBridge } from "../motion/motionViewportBridge";
...
function applySnapshotToCamera(camera: ThreePerspectiveCamera, snapshot: CameraShotSnapshot) {
  camera.fov = getVerticalFovFromFocalLength(snapshot.fov);
      ...
      const snapshot = (label: string) => {
        ...
        return {
          ...
          meta: buildScreenshotMeta({
            ...
            fov: getFocalLengthFromVerticalFov(workingCamera.fov),
  ...
  return (
    <div className="viewport-gizmo-overlay" aria-label="3D视口原生坐标控件" style={{ right: `${rightOffset}px` }}>
      <Canvas
        ...
        camera={{ fov: getVerticalFovFromFocalLength(snapshot.fov), position: [0, 0, 1] }}
        dpr={dpr}
    ...
    >
      ...
      <div className="motion-monitor-canvas-wrap" style={{ aspectRatio }}>
        <Canvas
          ...
          camera={{ fov: getVerticalFovFromFocalLength(monitorCamera.fov), position: monitorCamera.position }}
          dpr={renderDpr}
          gl={{ antialias }}
        >
          <ViewportBackground
      ...
      <div className="motion-monitor-fov" aria-label="看成片焦段设置">
        <label>
          <span>看成片焦段 (mm)</span>
          <input
            aria-label="看成片焦段"
          ...
          />
          <output>{Math.round(finishedShotFov ?? cameraSnapshot?.fov ?? 50)}mm</output>
        </label>
        <button type="button" disabled={finishedShotFov === null} onClick={() => onFinishedShotFovChange(null)}>
          跟随轨迹
        </button>
      </div>
      <div className="motion-monitor-fov motion-monitor-fov--secondary" aria-label="小窗焦段设置">
        <label>
          <span>小窗焦段 (mm)</span>
          <input
            aria-label="小窗焦段"
          ...
          />
          <output>{Math.round(monitorFov ?? monitorCameraBase?.fov ?? 50)}mm</output>
        </label>
...
export function DirectorCanvas() {
  ...
  useEffect(() => {
    setMotionViewportBridge({
      getSnapshot: getViewportCameraSnapshot,
      loadSnapshot: (snapshot) => updateDirectorViewSnapshot(snapshot, true),
      startPilot: startPilotSession,
    });

    return () => setMotionViewportBridge(null);
    // The bridge reads the latest camera snapshot through refs and is scoped to this canvas.
    ...
    <div className="canvas-frame" data-benchmark-mode={benchmarkMode ?? "off"}>
      <div className="director-canvas" data-testid="director-canvas" ref={viewportContainerRef}>
        <Canvas
          ...
          camera={{ position: directorViewSnapshot.position, fov: getVerticalFovFromFocalLength(directorViewSnapshot.fov) }}
        ...
        >
          ...
          <DirectorKeyboardController
            ...
            onCameraChange={(snapshot) => updateDirectorViewSnapshot(snapshot, true)}
          />
      ...
      {activeCameraView && referenceVideoRendering ? createPortal((() => {
        ...
        return (
          ...
          >
            <Canvas
              camera={{ fov: getVerticalFovFromFocalLength(activeCameraView.fov), position: activeCameraView.position }}
              dpr={1}
