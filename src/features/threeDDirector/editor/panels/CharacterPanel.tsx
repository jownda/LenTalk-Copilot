      ...
      {activeTab === "properties" ? (
        <>
          ...
          {!isCrowd ? (
            <TransformGizmoHeightControl
              ariaLabel="角色控制器高度"
              value={role.transformGizmoHeight}
              onChange={(height) => updateTransformGizmoHeight(role.id, height)}
            />
          ) : null}
          <InspectorColorField
            label="颜色"
        ...
        <InspectorSection title="动作预设" className="pose-preset-section">
          ...
          <div className="preset-grid">
            ...
            {allowsHumanoidActions ? visibleCharacterActionPresets.map((preset) => (
              <button
                key={preset.id}
                      ...
                      {selectedRoutePoint.holdAction === "custom" ? (
                        <label className="inspector-field">
                          ...
                          >
                            ...
                            {allowsHumanoidActions ? visibleCharacterActionPresets.map((preset) => <option key={preset.id} value={preset.id}>{preset.label}</option>) : null}
