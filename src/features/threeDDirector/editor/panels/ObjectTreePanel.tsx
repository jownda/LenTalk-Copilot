...
import { Box, Camera, ChevronDown, ChevronRight, Eye, EyeOff, Image as ImageIcon, Layers3, Lock, Search, Unlock, User, Users } from "lucide-react";
...
type ObjectTreeIconKind = "character" | "crowd" | "geometry" | "model" | "camera" | "billboard";
...
function ObjectKindIcon({ icon }: { icon: ObjectTreeIconKind }) {
  ...
  return (
    <span className="object-row-kind-icon" data-testid={`object-row-icon-${icon}`}>
      ...
      {icon === "billboard" ? <ImageIcon {...iconProps} /> : null}
    </span>
  );
}
...
export function ObjectTreePanel() {
  ...
  const groupedItems = useMemo(() => {
    ...
    return {
      ...
      billboards: regularItems.filter((item) => item.object?.kind === "billboard"),
  ...
  const filteredGroups = GROUP_LABELS.map((group) => {
    const itemsByGroup =
      group.key === "characters"
        ...
        : group.key === "crowd"
          ? groupedItems.crowd
          : group.key === "billboards"
            ? groupedItems.billboards
            : group.key === "geometry"
