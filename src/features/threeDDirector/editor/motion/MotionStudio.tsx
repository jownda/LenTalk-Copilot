...
export function MotionStudio({
  ...
  const addCameraVideoCapture = useDirectorStore((state) => state.addCameraVideoCapture);
  ...
  if ((!open && !embedded) || !activeCamera) return null;
  ...
  async function exportReferenceVideo() {
    ...
    try {
      ...
      const videoDataUrl = await postDirectorDeskReferenceVideoToHost(result);
      addCameraVideoCapture(activeCamera.id, videoDataUrl, result.fileName);
      setExportStatus("MP4 参考视频已添加到画布");
    } catch (error) {
  ...
  return (
    <section className={`motion-studio${embedded ? " is-embedded" : ""}${cameraPilotMode !== "idle" ? " is-piloting" : ""}`} aria-label="运镜工作台">
      ...
      {exportOpen ? (
        <section className="motion-export-panel" aria-label="导出运镜设置">
          ...
          <button type="button" className="motion-export-confirm" disabled={motionPath.keyframes.length < 2 || exporting} onClick={() => void exportReferenceVideo()}><Download aria-hidden="true" size={14} />{exporting ? "正在录制" : "导出MP4并添加到画布"}</button>
