...
import { getFocalLengthFromVerticalFov, getVerticalFovFromFocalLength } from "../schema/cameraGeometry";
...
const PILOT_MIN_FOV = getVerticalFovFromFocalLength(120);
const PILOT_MAX_FOV = getVerticalFovFromFocalLength(10);
...
export function CameraPilotController({
  ...
  useEffect(() => {
    ...
    pilotCamera.fov = getVerticalFovFromFocalLength(snapshotRef.current.fov);
  ...
  useFrame((_state, delta) => {
    ...
    const snapshot: CameraMotionSnapshot = {
      fov: Number(getFocalLengthFromVerticalFov(pilotCamera.fov).toFixed(3)),
