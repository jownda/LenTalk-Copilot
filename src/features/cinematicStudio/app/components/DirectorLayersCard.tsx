...
import { compileDirectorSequence, DIRECTOR_LAYERS, FINAL_GENERATED_DIRECTOR_LAYER_KEYS, type DirectorLayerKey } from "../../engine";
...
function FirstFrameReferenceSlots({ project, scene, t, canvasImageSources, setNotice, onUpdateScene, text, onTextChange, autoGeneratedText }: FirstFrameReferenceSlotsProps) {
  ...
  return <div className="first-frame-layer-editor" onClick={(event) => event.stopPropagation()}>
    {autoGeneratedText ? <div className="hint-text director-auto-layer-note">{autoGeneratedText}</div> : <textarea className="modal-textarea director-layer-textarea" style={{ paddingLeft: `${references.length > 0 ? Math.min(references.length + 1, 5) * 40 + 14 : 54}px` }} value={text} placeholder={t.directorLayerEmpty} onChange={(event) => onTextChange(event.target.value)} />}
...
export default function DirectorLayersCard({ project, scene, t, locale, canvasImageSources, setNotice, onUpdateScene, focusLayerKey }: DirectorLayersCardProps) {
  ...
  const generatedSequence = useMemo(() => compileDirectorSequence(project, scene, { locale }), [locale, project, scene]);
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => document.getElementById(`cinematic-director-layer-${focusLayerKey}`)?.scrollIntoView({ behavior: "smooth", block: "center" }));
  }, [focusLayerKey]);
  ...
  return <section className="card director-layers-card">
    ...
    <div className="director-layers">
      {DIRECTOR_LAYERS.map((layer) => {
        ...
        return <div id={`cinematic-director-layer-${layer.key}`} key={layer.key} className={`director-layer${isLocked ? " locked" : ""}`}>
          <div className="director-layer-head" onClick={() => setOpenKey(isOpen ? null : layer.key)}>
            {!isGeneratedOnly && <button className={`layer-lock${isLocked ? " active" : ""}`} title={isLocked ? t.unlockLayer : t.lockLayer} onClick={(event) => { event.stopPropagation(); toggleLock(layer.key); }}>
          ...
          {isOpen && layer.key === "firstFrame" && <FirstFrameReferenceSlots project={project} scene={scene} t={t} canvasImageSources={canvasImageSources} setNotice={setNotice} onUpdateScene={onUpdateScene} text={text} onTextChange={(value) => updateLayer(layer.key, value)} autoGeneratedText={text || t.firstFrameReferenceHint} />}
          {isOpen && layer.key === "activeReferences" && <div className="director-generated-layer-text">{text || t.directorLayerEmpty}</div>}
          {isOpen && !isGeneratedOnly && layer.key !== "firstFrame" && <textarea className="modal-textarea director-layer-textarea" value={text} placeholder={t.directorLayerEmpty} onChange={(event) => updateLayer(layer.key, event.target.value)} />}
        </div>;
      })}
    </div>
  </section>;
