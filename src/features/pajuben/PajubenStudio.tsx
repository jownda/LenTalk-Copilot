...
import { usePajubenModelOptions, groupPajubenModelOptions } from './pajubenModels';
...
export function PajubenStudio({ onClose }: { onClose: () => void }) {
   ...
   */
  const autoEpisode = useMemo(
    () => (!batch && target.trim() ? describeEpisodeGuess(target) : null),
    [batch, target]
  );
  ...
  const overallPercent =
    overall.total > 0 ? Math.min(100, Math.round((overall.done / overall.total) * 100)) : 0;
          ...
          >
            {logs.length === 0 ? (
              <p className="text-text-muted">
                {t('pajuben.consoleEmpty', '等待开始…引擎的输出会实时显示在这里')}
              </p>
            ) : (
              logs.map((entry) => (
                <div
...
function EnvironmentBadge({
  ...
  environment: PajubenEnvironment | null;
  probing: boolean;
}) {
  const { t } = useTranslation();
