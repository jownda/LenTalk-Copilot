...
/** 从节点中提取项目封面缩略图(前 4 张图片 URL,解码 imagePool 引用) */
function extractProjectThumbnails(nodes: CanvasNode[], imagePool?: string[]): string[] {
  ...
  const pushImage = (url: unknown) => {
    ...
    const decoded = decodeImageReference(url, imagePool) ?? url;
    if (decoded.startsWith(IMAGE_REF_PREFIX)) return; // 无法解码的引用跳过
  ...
  for (const node of nodes) {
    ...
    if (data.imageUrl) pushImage(data.imageUrl);
    else if (data.previewImageUrl) pushImage(data.previewImageUrl);
    if (Array.isArray(data.frames)) {
      for (const frame of data.frames as Array<Record<string, unknown>>) {
        if (frame.imageUrl) pushImage(frame.imageUrl);
      }
    }
...
/**
 * 从【已编码】节点中提取项目封面缩略图引用(前 4 个,去重)。
 * 只返回 `__img_ref__:N` 形式的轻量引用(几十字节),不携带图片本体,
 * 避免项目记录写入 localStorage 时因 4 张原图 data URL 重复导致超限丢失。
 * 读取时(浏览器降级的 listProjectSummaries)再从 historyJson 的 imagePool 解码。
 */
function extractProjectThumbnailRefs(nodes: CanvasNode[]): string[] {
  const refs: string[] = [];
  const seen = new Set<string>();

  const pushRef = (url: unknown) => {
    if (refs.length >= 4) return;
    if (typeof url !== 'string' || !url) return;
    // 只保留可解码的引用(编码节点中的图片一定是 __img_ref__:N 或 data URL,
    // 后者直接跳过,避免把大图写进 thumbnails)
    if (!url.startsWith(IMAGE_REF_PREFIX)) return;
  ...
  for (const node of nodes) {
    const data = node.data as Record<string, unknown>;
    if (data.imageUrl) pushRef(data.imageUrl);
    else if (data.previewImageUrl) pushRef(data.previewImageUrl);
    if (Array.isArray(data.frames)) {
      for (const frame of data.frames as Array<Record<string, unknown>>) {
        if (frame.imageUrl) pushRef(frame.imageUrl);
      }
    }
...
function mapNodeImageReferences(
  ...
  return nodes.map((node) => {
    ...
    // 360 全景节点:输入全景图与取景输出图
    if ('inputImageUrl' in nextData) {
      nextData.inputImageUrl = mapImageUrl(nextData.inputImageUrl as string | null | undefined) ?? null;
    }
    if ('previewInputImageUrl' in nextData) {
      nextData.previewInputImageUrl =
        mapImageUrl(nextData.previewInputImageUrl as string | null | undefined) ?? null;
    }
    if ('outputImageUrl' in nextData) {
      nextData.outputImageUrl = mapImageUrl(nextData.outputImageUrl as string | null | undefined) ?? null;
    }
    if ('outputPreviewImageUrl' in nextData) {
      nextData.outputPreviewImageUrl =
...
function toProjectSummary(record: ProjectSummaryRecord): ProjectSummary {
  return {
    ...
    ...(record.thumbnails && record.thumbnails.length > 0 ? { thumbnails: record.thumbnails } : {}),
  };
}
...
function toProjectRecord(project: Project): ProjectRecord {
  ...
  return {
    ...
    thumbnails: extractProjectThumbnailRefs(persistedNodes),
      ...
      try {
        ...
        set((state) => ({
          ...
          projects: updateProjectSummary(state.projects, {
            ...
            ...(() => {
              const imagePool = (project as PersistedProject).imagePool;
              const thumbs = extractProjectThumbnails(project.nodes, imagePool);
  ...
  closeProject: () => {
    ...
    if (currentProjectId && currentProject && currentProject.id === currentProjectId) {
      ...
      persistedSummary = {
        ...
        ...(() => {
          const imagePool = (nextProject as PersistedProject).imagePool;
          const thumbs = extractProjectThumbnails(nextProject.nodes, imagePool);
          return thumbs.length > 0 ? { thumbnails: thumbs } : {};
        })(),
      };
  ...
  saveCurrentProject: (nodes, edges, viewport, history) => {
    ...
    set((state) => ({
      ...
      projects: updateProjectSummary(state.projects, {
        ...
        nodeCount: nextProject.nodeCount,
        ...(() => {
          const thumbs = extractProjectThumbnails(nodes, (currentProject as PersistedProject).imagePool);
          return thumbs.length > 0 ? { thumbnails: thumbs } : {};
        })(),
      }),
    }));
