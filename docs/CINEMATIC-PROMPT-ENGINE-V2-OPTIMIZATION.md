...
| 步骤 | 内容 | 层 | 风险 | 提交点 |
|---|---|---|---|---|
| V2.8 | 布局滚动修复（高度链 100% + 清理坏选择器） | UI | 低 | `fix:` 已完成 |
| V2.9 | 模型下拉迁移 + 移除工作室全部设置入口（API 统一 LenTalk 设置 → 密钥） | UI | 低 | `feat:` 已完成 |
| V2.10 | 资产备注字段 + AI 填写纳入备注（角色信息栏） | 数据+AI+UI | 中 | `feat:` 已完成 |
...
| V2.15 | P3 回归走查（下节断言）+ 提交 | 验收 | — | `fix:` 自动化完成，待人工视觉复核 |
...
**V2.15 验收结果**：自动化回归完成。全量 19 个测试文件、85 条断言通过；类型检查和生产构建通过。额外修正了两项回归：AI 未配置提示统一指向 LenTalk「设置 → 自定义平台」及 AI 编译区模型下拉；AI 分镜 schema 直接移除了 `audioPlan` 与旧 `lens` 字段，只保留 `optics.fieldOfViewDegrees`。代码审计确认工作室主界面无设置入口、模型选择位于 AI 编译区左侧，主内容区可纵向滚动。由于当前执行环境拒绝启动本地预览服务器，滚动到底、下拉选择和编辑弹窗的浏览器视觉复核待在可启动预览的桌面环境完成；其余断言已由单测与构建覆盖。
...
### P4-22 层标题统一「标题：」分隔（防歧义）

**证据**：用户实测最终提示词中「场景地图」等层标题没有冒号。代码核查：
1. 结构化路径：`push()` 渲染为 `${header}\n${body}`（`director.ts` 的 `push`），标题后**无冒号**；
2. AI 分层路径：`ai.ts` 生成规则只要求「首行为本层段头」，**未规定冒号**，模型输出的段头时有时无；
3. 内部部分渲染器（资产行、技术段、音频、负面、严格身份锁）又自带 `标题：`，整体格式不统一；
4. 简报原文的块标题本身就带冒号（"OPTICS: 200mm long telephoto…"）。

**要点**：标题行是「块标签」。没有冒号时，模型与任何解析器都无法稳定区分「标题行」与「正文首行」——标题可能被当成画面指令读进去，「场景地图」单独成行还可能被当作画面元素；统一「中文全角：/ 英文半角 :」后，标签 → 内容的边界无歧义，校验器也能可靠识别层边界。

**改动**
1. 结构化路径：`push()` 统一改为 `${header}${zh ? "：" : ":"}\n${body}`（`NEGATIVE LOCKS (GLOBAL ONLY)` 同样加半角冒号）；
2. AI 分层路径：生成规则补充「段头后必须跟全角/半角冒号再换行」；`normalizeDirectorLayers` 对缺失冒号的段头自动补全；
3. 防双标题：body 若已以「自身标题：」开头（如 AUDIO 内层自带标题），补冒号时跳过，避免出现 `AUDIO\n音频：` 双行标题。

**文件**：`engine/compiler/director.ts`（`push`）、`app/providers/ai.ts`、`engine/continuity.ts`（回归校验）。
...
| 步骤 | 内容 | 层 | 风险 | 提交点 |
|---|---|---|---|---|
| V2.16 | 资产命名脚手架 + 状态变体数据模型（@类型_项目码_名称_状态_v版本） | 数据+UI | 中 | `feat:` 已完成 |
| V2.17 | 资产状态化 UI：变体切换 / 车辆内部归道具 / 人物表规范引导 / 压测状态 | UI | 中 | `feat:` 已完成 |
...
| V2.24 | 十五块骨架固化 + 无独立负面块 + LOCATION MAP 固定第二块 + 层标题统一「：/ :」（P4-22） | 引擎+AI | 高 | `refactor:` |
...
**V2.16 验收结果**：已完成。项目新增可编辑项目码；资产新增状态组、基卡、状态名、版本、变更记录与稳定引用标签。新资产默认建立 `base / v1` 基卡，旧项目加载时自动升级为同一结构。资产编辑页可查看引用并修改状态、版本、变更记录；项目码、资产名称、状态或版本变化会即时重算 `@类型_项目码_名称_状态_v版本`。编译资产行与镜头内 `@` 引用均读取稳定标签。新增迁移、Reducer、渲染测试。

**V2.17 验收结果**：已完成。资产编辑器可从任一资产创建独立状态变体；新卡继承参考图与身份信息，冻结基卡描述，只填写变化部分，导出时自动合并基卡描述与变化描述。新增压测状态（未测 / 通过 / 失败）、车辆内部地点转道具入口，以及角色人物表规范引导。新增变体与描述合并回归测试。
...
12. 导出的每个层标题行以全角「：」（zh）/ 半角「:」（en）结尾，正文紧随其后；不存在 `AUDIO\n音频：` 式双标题。
