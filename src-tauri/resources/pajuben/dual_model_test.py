...
场景首次出现时必须记录场面调度：人物在画面左/中/右、前/中/后景，坐/站、朝向、与门/桌/床等可见参照物的关系；人物换位、进出、靠近/拉开或转身时，必须另起条记录变化。多人同框分别写清站位，不能笼统写“几人对峙”。
表情和情绪只写可见变化及证据，例如“眉头收紧、后退半步、攥住衣角”，可归纳为“戒备（后退半步，手护在身前）”；看不清就不写，严禁脑补心理。
...
- 每场开头必须输出“场面调度：”，记录可见的左右/前后景站位、朝向、坐站和相对位置；站位变化、进出、靠近/分开或转身必须在对应△中记录。
- 每场可见状态有明确证据时，必须输出“人物状态：角色：状态（表情/动作证据）”；情绪只能根据可见证据归纳，不能编造心理活动。
- 每条△都带【分:秒】时间戳，动作按“起势→过程→结果/对方反应”写清主体、对象和方向。
