...
### 12.1 全局素材库（Asset Library）

- 位置：`src/features/library/`（`types.ts` / `assetStore.ts` / `importAssets.ts` / `AssetLibraryPanel.tsx`）。
- 定位：跨项目全局数据，**不写入项目 SQLite**，独立持久化在 `localStorage`（key `storyboard-asset-library-v1-v1`）。
- 数据模型：`LibraryAsset { id, name, categoryId, imageUrl, previewImageUrl, aspectRatio, sourceFileName, createdAt }`；`AssetCategory { id, name, createdAt }`。
- 图片导入复用 `prepareNodeImageFromFile`（`application/imageData.ts`），在 Tauri 下自动落盘为本地路径；面板通过 `resolveImageDisplayUrl` 渲染预览。
- 插入画布：面板点击素材 → 读取 `useAssetLibraryStore.getState().assets` → `useCanvasStore.addNode(CANVAS_NODE_TYPES.upload, position, data)`，位置取视口中心（`resolveViewportCenterPosition`）。
- 持久化写失败时（localStorage 满）自动降级：剥离超过 4MB 的 `imageUrl` 原始 dataURL，仅保留 `previewImageUrl`。

### 12.2 视频帧抽取（Video Frame Extract）

- 位置：`src/features/canvas/ui/VideoFrameExtractDialog.tsx`；入口在 `Canvas.tsx` 右上角按钮组。
- 纯前端实现：`<video>` 元素 + canvas `drawImage` 抽帧（`drawVideoFrame`，最长边 1920px，JPEG 0.9）。
- 结果可「插入画布」或「存入素材库」；两者都先走 `prepareNodeImage` 获取持久化路径与真实宽高比，再创建 upload 节点 / 写入素材库。

### 12.3 360 全景查看（Panorama Viewer）

- 位置：`src/features/canvas/ui/PanoramaViewer.tsx`；入口在 `ImageViewerModal` 底部控制条的「360 全景查看」按钮。
- 纯前端 CSS 3D：`perspective` 容器 + `rotateX/rotateY` 拖拽旋转 + 滚轮调节 FOV（`scale = 1/tan(fov/2)`）。
- 打开全景模式时返回独立全屏层，`Escape` 或「退出全景」按钮返回普通查看。

### 12.4 循环节点（Loop Node）

- 暂缓实现：Storyboard-Copilot 为单向节点链（无执行引擎），Infinite Canvas 的循环节点依赖其自研执行流，直接移植需引入执行引擎概念，工程量过大。后续如需，建议先以「批量循环生成」形态落地。

### 12.5 360 全景节点（Panorama Node）

- 位置：`src/features/canvas/nodes/PanoramaNode.tsx`；类型注册在 `canvasNodes.ts` / `nodeRegistry.ts` / `nodeDisplay.ts` / `nodes/index.ts`。
- 数据模型：`PanoramaNodeData` 关键字段 `inputImageUrl`（输入全景图）、`yaw/pitch/fov`（视角状态，随拖拽实时持久化）、`outputAspect`（输出画幅 16:9/1:1/9:16/21:9/4:3）、`outputImageUrl`（取景输出图）。
- 渲染：Three.js `SphereGeometry(500,96,64)` 内表面贴图 + 相机球心 lookAt；拖拽改 yaw/pitch（系数 0.18，±85°），滚轮改 fov（35~100，系数 0.08）。
- 输入：支持拖入图片文件（写入 `inputImageUrl`）或从上游连线（`graphImageResolver.collectInputImages` 收集）。取景输出后 source 端对外提供 `outputImageUrl`（未取景时回退输入全景图）。
- 取景输出：`renderFramedOutput` 用离屏 renderer 按 `outputAspect` 渲染当前视角（最长边 2048px，PNG），再走 `prepareNodeImage` 持久化。
- 持久化：`projectStore.mapNodeImageReferences` 已扩展编码 `inputImageUrl / previewInputImageUrl / outputImageUrl / outputPreviewImageUrl` 四个字段（imagePool 去重）。

### 12.6 旋转工具（Rotate Tool）

- 位置：`src/features/canvas/tools/builtInTools.ts`（`rotateToolPlugin`）、`src/features/canvas/ui/tool-editors/RotateToolEditor.tsx`。
- 操作类型：`rotate-left-90 / rotate-right-90 / rotate-left-180 / rotate-right-180 / flip-horizontal / flip-vertical`（存于 options.operation）。
- 编辑器：6 个操作按钮，点击即在前端 canvas 即时预览（`applyTransform` 先复制原 canvas 到临时 canvas 再变换，避免自绘失真）。
- 执行：`application/toolProcessor.ts` 的 `rotateImage`（前端 canvas 变换，旋转交换宽高、镜像用 scale(-1,1)/(1,-1)），产物走 `addDerivedExportNode` 生成结果节点。
- 图标映射：`NodeActionToolbar.tsx` 的 `toolIconMap` 增加 `rotate: RotateCw`；`types.ts` 的 `ToolIconKey` / `ToolEditorKind` 增加 `rotate`。

### 12.7 提示词库（Prompt Library）

- 位置：`src/features/prompts/promptLibraryStore.ts`（store）、`src/features/prompts/PromptLibraryPanel.tsx`（面板，画布工具栏「提示词库」入口）。
- 数据模型（对齐 Infinite Canvas）：多词库 `libraries[]` → 条目 `items[]`，条目字段 `name / scene(用途说明) / positive(正向) / negative(负向) / category`，localStorage key `storyboard-prompt-library-v2`，自动迁移旧版 v1 数据。
- UI：三栏布局（左词库树含新建/重命名/删除；中列表含搜索/分类 chips/批量管理模式；右详情预览/内联表单编辑）。双击条目或「应用到节点」把正向提示词写入 imageEdit 节点 prompt（负向字段仅存储，节点不支持）。
- 内置案例：`src/features/prompts/builtinPromptSeeds.ts` 由 Infinite Canvas 的 `static/system-prompts/infinite-canvas-prompt-templates.md` 解析生成（10 个预设，含适用场景/正向/负向/分类），运行时自动合并为只读词库「无限画布预设」（`lib-builtin-canvas-prompts`，增删改跳过、不持久化、条目随种子文件更新）；面板只读时隐藏新增/编辑/删除/批量管理并显示「内置案例」标识。
- 注意：应用时若已选中 imageEdit 节点则追加 `, ${positive}` 到现有 prompt，否则新建 imageEdit 节点。
